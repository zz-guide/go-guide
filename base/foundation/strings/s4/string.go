...
int====uint====32为操作系统为uint32,64为操作系统为uint64
byte====uint8
rune====uint32
...
func TestJoinString() {
	...
	for i := 0; i < k-1; i++ {
		fmt.Printf("方式 %d is %6.1f times of 方式 %d\n", i, float32(d[i])/float32(d[k-1]), k-1)
	}

	/**
	运行结果：
		4100000
		time of way(0)=49.317940779s
		4100000
		time of way(1)=26.566608673s
		4100000
		time of way(2)=1m7.424187095s
		4100000
		time of way(4)=4.96834ms
		方式 0 is 9926.4 times of 方式 4
		方式 1 is 5347.2 times of 方式 4
		方式 2 is 13570.8 times of 方式 4
		方式 3 is    0.0 times of 方式 4

	*/
	//结论：
	//string + 比join和sprintf快，但最快的是buffer，并且是只申明一次的buffer最快,10万次只有几毫秒
	//如果循环内每次都临时声明一个bytes.Buffer来使用，会慢很多
}
...
func benchmarkStringFunction(n int, index int) (d time.Duration) {
	v := "qing jiao wo lu xun ni shuo hao bu hao?" //请叫我鲁迅，你说好不好
